en:
  ...
  Privacy: Privacy
  General: General
  use slot for complexed content: use slot for complexed content
  slot content: slot content
  'Hi, I''m Vux.': 'Hi, I''m Vux.'
  isLink is set to true when link exists: isLink is set to true when link exists
  ...
  Toggle_show_input: Toggle_show_input
  ...
  Used as a plugin(Hide in 2s): Used as a plugin(Hide in 2s)
...
zh-CN:
  ...
  Privacy: 隐私
  General: 通用
  use slot for complexed content: 使用slot显示复杂内容
  slot content: slot 内容
  'Hi, I''m Vux.': 你好哇，感谢关注 Vux
  isLink is set to true when link exists: 当有设置link属性时，isLink属性会设为true以显示箭头
  ...
  handle errors: 处理错误
  preselect China and Japan(disabled): 默认选中China和Japan(禁用操作)
  set max=2: 最多可选中2个
  set random order: 打乱选项顺序
  Option Array with key and value(key must be string): 使用Object类型的选项列表，key必须为字符串
  Async list: 异步选项列表
  ...
  'use prop:show.sync(vue^2.3) to control visibility': '使用 prop:show 控制显示(vue^2.3)'
  Used as a plugin(Hide in 2s): 插件形式调用
